  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={FileText} label="Jami hujjatlar" value={data?.total ?? 0} />
        <StatCard icon={Clock} label="Ko'rib chiqilmoqda" value={inReview} accent="text-amber-600 bg-amber-50" />
        <StatCard icon={AlertCircle} label="Imzo kutilmoqda" value={pendingSignature} accent="text-sky-600 bg-sky-50" />
        <StatCard icon={CheckCircle2} label="Imzolangan" value={signed} accent="text-emerald-600 bg-emerald-50" />
      </div>
    </div>
  );
}
