...
import { Archive, FileText, QrCode, Trash2 } from "lucide-react";
...
export function LetterArchivePage() {
  ...
  return <div className="p-8"><div className="mb-6 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-100 text-brand-800"><Archive size={20}/></div><div><h1 className="font-display text-2xl font-semibold text-brand-950">{t("archive.deletedTitle")}</h1><p className="text-sm text-slate-500">{t("archive.deletedSubtitle")}</p></div></div><div className="overflow-hidden rounded-xl border border-slate-200 bg-white"><table className="w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">{t("letters.colNumber")}</th><th className="px-4 py-3">{t("letters.colTo")}</th><th className="px-4 py-3">{t("letters.colStatus")}</th><th className="px-4 py-3">{t("letters.colDate")}</th><th className="px-4 py-3">{t("archive.colFile")}</th></tr></thead><tbody className="divide-y divide-slate-100">{isLoading ? <tr><td colSpan={5} className="px-4 py-8 text-center">{t("documents.loading")}</td></tr> : data?.map((x:any)=>{
    ...
    return <tr key={x.id}>
      ...
      <td className="px-4 py-4">{!isDeleted ? <div className="flex gap-2">
        <button onClick={()=>download(x.id, `xat-${x.documentNumber}.docx`)} className="flex items-center gap-1.5 rounded-lg bg-sky-600 px-3 py-2 text-xs font-medium text-white hover:bg-sky-500"><FileText size={13}/> Word</button>
