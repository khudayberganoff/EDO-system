...
import { Archive, Download, QrCode, Trash2 } from "lucide-react";
import { downloadLetter, fetchArchive } from "../api/letters";
...
export function LetterArchivePage() {
  ...
  return <div className="p-8"><div className="mb-6 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-100 text-brand-800"><Archive size={20}/></div><div><h1 className="text-xl font-semibold">Fayllar arxivi</h1><p className="text-sm text-slate-500">Rahbariyat tasdiqlagan va o'chirilgan xatlar</p></div></div><div className="overflow-hidden rounded-xl border border-slate-200 bg-white"><table className="w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Raqam</th><th className="px-4 py-3">Kimga</th><th className="px-4 py-3">Holati</th><th className="px-4 py-3">Sana</th><th className="px-4 py-3">Fayl</th></tr></thead><tbody className="divide-y divide-slate-100">{isLoading ? <tr><td colSpan={5} className="px-4 py-8 text-center">Yuklanmoqda...</td></tr> : data?.map((x:any)=>{
    ...
    return <tr key={x.id}>
      ...
      <td className="px-4 py-4">{isDeleted ? <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600"><Trash2 size={12}/> O'chirilgan</span> : <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-800"><QrCode size={12}/> Tasdiqlangan</span>}</td>
      ...
      <td className="px-4 py-4">{x.finalFileUrl ? <button onClick={()=>download(x.id, x.finalFileUrl?.split('/').pop() || `xat-${x.documentNumber}.docx`)} className="flex items-center gap-2 rounded-lg bg-brand-800 px-3 py-2 text-xs font-medium text-white"><Download size={14}/> Yuklab olish</button> : <span className="text-slate-400">—</span>}</td>
    </tr>;
  })}</tbody></table></div></div>;
}
